            ...
            Thermo &thermo, Integrate &integrate, Timer &timer, int screen_yaml)
{
  ...
  hid_t       filespace, memspace, filespace1;      /* file and memory dataspace identifiers */
  hsize_t     dimsf;                 /* dataset dimensions */
  hsize_t     count;                 /* hyperslab selection parameters */
  hsize_t     offset;
  hid_t       plist_id, dataspace_id, attribute_id;             /* property list identifier */
  hsize_t     dims;
  herr_t      status;
  double *data;
  ...
  int maxatoms;
  MPI_Allreduce(&atom.nlocal, &maxatoms, 1, MPI_INT, MPI_MAX, MPI_COMM_WORLD);
   ...
   */
  dimsf = maxatoms*nprocs*3;
  filespace = H5Screate_simple(1, &dimsf, NULL);

  /*
   * Create the dataset with default properties and close filespace.
   */
  hid_t dset_id_x = H5Dcreate(file_id, "/atom.x", H5T_NATIVE_DOUBLE, filespace,
                        H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT);
  hid_t dset_id_v = H5Dcreate(file_id, "/atom.v", H5T_NATIVE_DOUBLE, filespace,
                        H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT);
  hid_t dset_id_f = H5Dcreate(file_id, "/atom.f", H5T_NATIVE_DOUBLE, filespace,
                        H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT);
  H5Sclose(filespace);
   ...
   */
  hid_t filespace_x = H5Dget_space(dset_id_x);
  hid_t filespace_v = H5Dget_space(dset_id_v);
  hid_t filespace_f = H5Dget_space(dset_id_f);
  H5Sselect_hyperslab(filespace_x, H5S_SELECT_SET, &offset, NULL, &count, NULL);
  H5Sselect_hyperslab(filespace_v, H5S_SELECT_SET, &offset, NULL, &count, NULL);
  H5Sselect_hyperslab(filespace_f, H5S_SELECT_SET, &offset, NULL, &count, NULL);
  ...
  status = H5Dwrite(dset_id_x, H5T_NATIVE_DOUBLE, memspace, filespace_x,
                      plist_id, atom.x);
  
  status = H5Dwrite(dset_id_v, H5T_NATIVE_DOUBLE, memspace, filespace_v,
                      plist_id, atom.v);

  status = H5Dwrite(dset_id_f, H5T_NATIVE_DOUBLE, memspace, filespace_f,
                      plist_id, atom.f);
  //free(data);
  ...
  H5Sclose(filespace_v);
  H5Sclose(filespace_f);
  H5Sclose(memspace);
  H5Pclose(plist_id);
   ...
   * Create the dataset with default properties and close filespace.
   */
  dset_id = H5Dcreate(file_id, "/atoms", H5T_NATIVE_INT, filespace,
			H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT);
  ...
  int localatoms[4] ;
  MPI_Allgather(&atom.nlocal, 1, MPI_INT, &localatoms, 1, MPI_INT, MPI_COMM_WORLD);

  H5LTset_attribute_int(file_id, "/atoms", "localatoms", localatoms, nprocs );

  char attr[256];
  sprintf(attr, VARIANT_STRING);
  H5LTset_attribute_string(file_id, "/atoms", "variant", attr );

  int natoms;
  MPI_Allreduce(&atom.nlocal, &natoms, 1, MPI_INT, MPI_SUM, MPI_COMM_WORLD);

  H5LTset_attribute_int (file_id, "/atoms", "natoms", &natoms, 1 );
  
  H5LTset_attribute_double(file_id, "/atoms", "density", &in.rho, 1);

  H5LTset_attribute_int (file_id, "/atoms", "mpi_procs", &nprocs, 1 );
  
  sprintf(attr,"%s",in.units == 0 ? "LJ" : "METAL");
  H5LTset_attribute_string (file_id, "/atoms", "units", attr  );

  H5LTset_attribute_int (file_id, "/atoms", "atom_types", &atom.ntypes, 1 );
  
  sprintf(attr,"%2.2lf %2.2lf %2.2lf",atom.box.xprd, atom.box.yprd, atom.box.zprd);
  H5LTset_attribute_string (file_id, "/atoms", "system_size", attr );

  sprintf(attr,"%i %i %i", in.nx, in.ny, in.nz);
  H5LTset_attribute_string (file_id, "/atoms", "unit_cells", attr );
